    ...
    if not tasks:
        ...
        for t in tasks:
            ...
            <div style='border-left: 5px solid {status_color}; padding: 15px; margin-bottom: 15px; background: #fafafa; border: 1px solid #eee; border-radius: 8px;'>
                ...
                <div style='margin-top: 5px; color: {status_color}; font-weight: bold; font-size: 14px;'>⏰ <b>Full Details:</b><br>{t['date']}</div>
            </div>
            """
...
def scrape_day_tasks(page, url):
    ...
    try:
        ...
        for event in event_elements:
            ...
            # Pura text uthao aur "Add submission" jaisa faltu text saaf kardo
            full_info = event.inner_text().replace("Add submission", "").strip()
            ...
            tasks.append({'name': name, 'course': course, 'date': full_info})
    except: pass
    return tasks
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if 9 <= current_hour <= 11:
                if tasks_today: send_professional_email("LMS URGENT: Due Today!", "Task Alert: Due Today!", "Action required by tonight.", tasks_today, "#d32f2f")
                else: send_professional_email("LMS Status: All Clear ✅", "Good Morning Asim!", "No deadlines found for today.", [], "#2e7d32")
                ...
                elif tasks_tomorrow: send_professional_email("LMS Alert: Due Tomorrow!", "Deadline Tomorrow", "You have a task due tomorrow.", tasks_tomorrow, "#673ab7")
                elif tasks_upcoming: send_professional_email("LMS Alert: 2 Days Left", "Upcoming Deadline", "Due in 2 days.", tasks_upcoming, "#0277bd")
                else: send_professional_email("LMS Evening Status ✅", "All Clear", "No urgent deadlines found.", [], "#2e7d32")
